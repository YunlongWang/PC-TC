...
if isempty(right_maximas) %if empty, rest of R-plot is a cluster
    ...
    if isempty(min_valley) % yunlong 20171129
        [min_valley, min_valley_indx] = min(ord_RD(cur_pts));
    end
    ...
    for i = 1:length(right_maximas)
        ...
        temp_idx = cur_max_indx+minpts+1;% yunlong 20171130
        if cur_max_indx+minpts+1>length(ord_RD)
            temp_idx = length(ord_RD);
        end
        if (cur_max * sig_pct < mean(ord_RD(cur_max_indx:temp_idx))) %if current maxima is not signifcant to the right of maxima
            cur_max = prev_max;
